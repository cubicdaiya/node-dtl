...
void Dtl::run(enum op_t op, long long *ed)
{
...
{
    HandleScope scope;
    Local<FunctionTemplate> t = FunctionTemplate::New(Dtl::New);
    t->InstanceTemplate()->SetInternalFieldCount(1);
    t->SetClassName(String::NewSymbol("dtl"));
    
    NODE_SET_PROTOTYPE_METHOD(t, "compose",             Dtl::Compose);
    NODE_SET_PROTOTYPE_METHOD(t, "composeUnifiedHunks", Dtl::ComposeUnifiedHunks);
    NODE_SET_PROTOTYPE_METHOD(t, "editdistance",        Dtl::Editdistance);
    NODE_SET_PROTOTYPE_METHOD(t, "printSES",            Dtl::PrintSes);
    NODE_SET_PROTOTYPE_METHOD(t, "printUnifiedFormat",  Dtl::PrintUnifiedFormat);

    target->Set(String::New("Diff"), t->GetFunction());
}
